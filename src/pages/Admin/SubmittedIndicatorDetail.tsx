...
import {
  ...
  XCircle, FileText, AlertTriangle, ShieldCheck, TrendingUp, Eye, Gavel,
  History, Save, Edit3, MessageSquare, Clock
} from "lucide-react";
...
const SubmittedIndicatorDetail: React.FC = () => {
  ...
  // --- LOGIC: Retrieve the latest rejection reason from the notes array ---
  const lastAdminNote = useMemo(() => {
    if (!indicator || !indicator.notes || indicator.notes.length === 0) return null;
    // Get the last note in the sequence (the most recent rejection justification)
  ...
  useEffect(() => {
    if (indicator) {
      setSelectedProgress(indicator.progress);
    }
  ...
  const handleUpdateEvidenceDescription = async () => {
    ...
    try {
      ...
      toast.success("Document note updated.");
      setEditingFile(null);
    } catch (err: any) {
  ...
  const handleApprove = async () => {
    ...
    try {
      await dispatch(submitIndicatorScore({ indicatorId: indicator._id, score: selectedProgress })).unwrap();
      await dispatch(approveIndicator({ id: indicator._id, notes: `Verified by ${currentUser?.role}` })).unwrap();
  ...
  return (
    <div className="min-h-screen p-6 lg:p-10 bg-[#f8f9fa] space-y-8 max-w-7xl mx-auto font-sans">
      ...
      <div className="flex justify-between items-center">
        <button onClick={() => navigate(-1)} className="group flex items-center gap-2 text-[#8c94a4] hover:text-[#1a3a32] font-black text-[10px] uppercase tracking-widest">
        ...
        <div className="flex gap-3">
          {submissionTimeInfo?.isResubmitted && (
            <div className="px-4 py-2 rounded-xl text-[10px] font-black uppercase bg-orange-100 text-orange-700 border border-orange-200 flex items-center gap-2">
              <History size={14} /> Resubmitted Attempt
            </div>
          )}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        <div className="lg:col-span-2 space-y-8">
          ...
          {/* REJECTION HISTORY PANEL: Displays the admin reason on resubmission */}
          {indicator.rejectionCount > 0 && lastAdminNote && (
            <div className="bg-rose-50 rounded-[2rem] p-8 border border-rose-100 flex gap-5">
              ...
              <div className="space-y-2">
                <h4 className="text-[10px] font-black text-rose-800 uppercase tracking-[0.2em]">Previous Revision Request</h4>
                <p className="text-sm text-rose-700 leading-relaxed font-medium italic">
                  "{lastAdminNote.text}"
                </p>
                <p className="text-[9px] font-bold text-rose-400 uppercase">
                  Logged on {new Date(lastAdminNote.createdAt).toLocaleString()}
                </p>
              </div>
            </div>
          )}
          ...
          <div className="bg-white rounded-[2.5rem] p-10 border border-slate-100 shadow-sm">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
              <div className="flex items-center gap-2 text-[#c2a336] font-black uppercase text-[10px] tracking-widest">
                <ShieldCheck size={14} /> Audit #ID-{indicator._id.slice(-6).toUpperCase()}
              </div>
              <div className="flex items-center gap-4 text-[10px] font-bold text-slate-400 uppercase">
                <div className="flex items-center gap-1.5"><Clock size={12}/> First: {submissionTimeInfo?.submittedAt}</div>
                <div className="w-1 h-1 bg-slate-200 rounded-full" />
                <div className="flex items-center gap-1.5 text-[#1a3a32]"><History size={12}/> Updated: {submissionTimeInfo?.lastModified}</div>
              </div>
            </div>
        ...
        <div className="bg-[#1a3a32] rounded-[3rem] p-8 text-white shadow-2xl space-y-8 sticky top-10">
           <div className="space-y-4">
            <h3 className="text-[10px] font-black uppercase tracking-widest text-[#c2a336]">Registry Controls</h3>
            <p className="text-xs text-slate-300 italic">Verify documents against registry requirements before final certification.</p>
          </div>

          <div className="space-y-4">
             <label className="text-[9px] font-black uppercase tracking-widest text-slate-400">Audit Completion Value (%)</label>
             <input 
              type="range" 
              min="0" max="100" 
              ...
              disabled={isFinalized}
              className="w-full accent-[#c2a336]" 
             />
             <div className="text-2xl font-black text-[#c2a336]">{selectedProgress}%</div>
          </div>
          ...
          <div className="space-y-3 pt-6 border-t border-white/10">
            <button 
              ...
              disabled={isFinalized} 
              className={`w-full flex items-center justify-center gap-3 py-5 rounded-2xl font-black text-[11px] uppercase tracking-widest transition-all ${isFinalized ? "bg-gray-600 text-slate-400" : "bg-[#c2a336] text-[#1a3a32] hover:bg-[#d4b44a]"}`}
            >
              <Gavel size={18} /> {isSuperAdmin ? "Certify & Close" : "Approve Exhibits"}
            </button>
            <button 
              onClick={() => setShowRejectModal(true)} 
              disabled={isFinalized} 
              className="w-full flex items-center justify-center gap-3 py-5 rounded-2xl font-black text-[11px] uppercase bg-white/10 hover:bg-white/20 text-white border border-white/10 disabled:opacity-30"
            >
              <XCircle size={18} /> Flag for Revision
            </button>
          </div>
        </div>
      </div>
      ...
      {showRejectModal && (
        <RejectModal reason={rejectReason} setReason={setRejectReason} onConfirm={handleReject} onClose={() => setShowRejectModal(false)} />
      )}
...
const LoadingState = () => (
  <div className="flex flex-col justify-center items-center h-screen bg-[#f8f9fa]">
    ...
    <p className="text-[#8c94a4] font-black uppercase tracking-widest text-[10px]">Registry Accessing...</p>
  </div>
);
...
const EmptyEvidence = () => (
  <div className="p-12 text-center border-2 border-dashed border-slate-100 rounded-[2rem]">
    ...
    <p className="text-[10px] font-black text-slate-400 uppercase">No verification exhibits found</p>
  </div>
);
...
const EditEvidenceDescriptionModal = ({ fileName, description, setDescription, onConfirm, onClose, loading }: any) => (
  <div className="fixed inset-0 bg-[#1a3a32]/80 backdrop-blur-sm flex justify-center items-center z-50 p-4">
    <div className="bg-white rounded-[2.5rem] p-10 w-full max-w-lg shadow-2xl">
      <h2 className="text-2xl font-black uppercase mb-2 flex items-center gap-3"><Edit3 className="text-[#c2a336]" /> Document Annotation</h2>
      <p className="text-[10px] text-slate-400 mb-6 uppercase font-black tracking-widest truncate">Editing Note for: <span className="text-[#c2a336]">{fileName}</span></p>
      <textarea
        value={description}
        onChange={(e) => setDescription(e.target.value)}
        placeholder="Enter specific comments regarding this document..."
        className="w-full border border-slate-100 bg-slate-50 rounded-2xl p-6 mb-8 focus:ring-2 focus:ring-[#c2a336] outline-none min-h-[150px] text-sm"
      />
      <div className="flex gap-4">
        <button onClick={onClose} className="flex-1 font-black uppercase text-[10px] text-slate-400">Cancel</button>
        <button onClick={onConfirm} disabled={loading} className="flex-[2] py-4 rounded-2xl bg-[#1a3a32] text-white text-[11px] font-black uppercase flex items-center justify-center gap-2 shadow-lg shadow-[#1a3a32]/20">
          {loading ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />} Save Annotation
        </button>
      </div>
    </div>
  </div>
...
const RejectModal = ({ reason, setReason, onConfirm, onClose }: any) => (
  <div className="fixed inset-0 bg-[#1a3a32]/80 backdrop-blur-sm flex justify-center items-center z-50 p-4">
    <div className="bg-white rounded-[2.5rem] p-10 w-full max-w-lg shadow-2xl">
      <h2 className="text-2xl font-black uppercase mb-6 text-rose-600 flex items-center gap-3"><AlertTriangle /> Revision Request</h2>
      <textarea
        value={reason}
        onChange={(e) => setReason(e.target.value)}
        placeholder="Reason for requesting revision..."
        className="w-full border border-slate-100 bg-slate-50 rounded-2xl p-6 mb-8 focus:ring-2 focus:ring-rose-500 outline-none min-h-[150px] text-sm"
      />
      <div className="flex gap-4">
        <button onClick={onClose} className="flex-1 font-black uppercase text-[10px] text-slate-400">Cancel</button>
        <button onClick={onConfirm} className="flex-[2] py-4 rounded-2xl bg-rose-600 text-white text-[11px] font-black uppercase shadow-lg shadow-rose-600/20">Send Back</button>
      </div>
    </div>
  </div>
);
