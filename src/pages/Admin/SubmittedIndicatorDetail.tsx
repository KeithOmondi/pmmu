...
const SubmittedIndicatorDetail: React.FC = () => {
  ...
  const custodians = useMemo(() => {
    ...
    if (indicator.assignedToType === "individual") {
      return (
        users.find((u) => u._id === indicator.assignedTo)?.name ||
            ...
            {indicator.evidence?.filter((e) => !e.isArchived).length === 0 ? (
              ...
              <ul className="space-y-4">
                {indicator.evidence?.filter((e) => !e.isArchived).map((file, idx) => (
                  <li key={file._id || `file-${idx}`} className="p-5 flex items-start justify-between bg-white border border-slate-100 rounded-3xl hover:border-[#c2a336]/30 transition-all group">
