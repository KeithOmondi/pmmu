          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-white rounded-[2rem] border border-gray-100 p-8 shadow-sm h-full">
              <h2 className="text-xs font-black text-[#1a3a32] uppercase tracking-[0.2em] mb-6 flex items-center gap-2">
                <History size={16} className="text-[#c2a336]" />
                Evidence Archive
              </h2>
              ...
              {indicator.evidence.length === 0 ? (
                ...
                <ul className="space-y-3">
                  {indicator.evidence.map((file: any, index: number) => (
                    <li
                      key={index}
                      className="flex items-center gap-3 p-4 bg-[#f8f9fa] rounded-2xl border border-gray-50 text-[#1a3a32] font-bold text-sm"
                    >
                      <div className="p-2 bg-white rounded-lg shadow-sm text-[#c2a336]">
                        <FileCheck size={16} />
                      </div>
                      <span className="truncate">
                        {file.name || `Exhibit_${index + 1}.pdf`}
                      </span>
                    </li>
                  ))}
                </ul>
                ...
                {selectedFiles.length > 0 && (
                  <div className="space-y-2 max-h-56 overflow-y-auto pr-2 custom-scrollbar">
                    <p className="text-[10px] font-black text-[#c2a336] uppercase tracking-widest mb-3">
                      Staged for Submission ({selectedFiles.length})
                    </p>
                    {selectedFiles.map((file, index) => (
                      <div
                        key={index}
                        className="flex items-center justify-between text-[11px] font-bold bg-white/5 px-4 py-3 rounded-xl border border-white/5 animate-in slide-in-from-right-2 duration-200"
                      >
                        ...
                        <button
                          ...
                          className="p-1 hover:bg-rose-500/20 rounded-md text-rose-400 transition-colors"
                        >
                          <X size={14} />
                        </button>
                      </div>
              ...
              <div className="mt-8 pt-6 border-t border-white/5">
                <button
                  ...
                  className="w-full flex items-center justify-center gap-3 px-6 py-4 rounded-2xl
                    bg-[#c2a336] text-[#1a3a32] text-[10px] font-black uppercase tracking-[0.2em] 
                    disabled:opacity-30 disabled:cursor-not-allowed transition-all shadow-lg active:scale-[0.98]"
                >
                  <UploadCloud size={16} />
                  Submit{" "}
                ...
                <p className="text-[9px] text-white/30 mt-4 text-center font-bold uppercase tracking-widest">
                  Official Record Submission Policy Applies
                </p>
              </div>
            </div>
          </div>
...
const DetailItem = ({ label, value, icon }: any) => (
  <div className="space-y-1">
    <div className="flex items-center gap-1.5 text-[10px] font-black text-[#8c94a4] uppercase tracking-widest">
      <span className="text-[#c2a336]">{icon}</span>
      {label}
    </div>
