    ...
    try {
          ...
          contents.forEach((relativePath, zipEntry) => {
            if (
              ...
              const p = zipEntry.async("blob").then((blob) => {
                ...
                const newFile = new File([blob], zipEntry.name, { type: mimeType });
      ...
      for (const f of extractedList) {
        ...
        if (!ALLOWED_EXT.includes(ext)) {
          toast.error(`Invalid type: ${f.name}`, { id: toastId });
          return;
        }
        if (f.size > MAX_SIZE) {
          toast.error(`${f.name} is too large (>5MB)`, { id: toastId });
          return;
        }
      }
      ...
      toast.success("Files ready for upload", { id: toastId });
    } catch (error) {
      toast.error("Error reading files", { id: toastId });
    }
  };
      ...
      <div className="max-w-7xl mx-auto p-6 md:p-12">
        ...
        <header className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-end mb-16">
          ...
          <div className="lg:col-span-4">
            <div className={`rounded-[2rem] p-8 text-white shadow-2xl relative overflow-hidden group transition-colors duration-500 ${isRevision && indicator.status === "rejected" ? "bg-orange-600 shadow-orange-900/20" : "bg-[#1E3A2B] shadow-emerald-950/20"}`}>
              <div className="absolute top-0 right-0 p-8 opacity-10 group-hover:scale-110 transition-transform duration-700">
                <ShieldCheck size={120} />
              </div>
              <p className="text-[10px] font-black uppercase tracking-[0.3em] opacity-60 mb-2">Audit Completion</p>
        ...
        {indicator.status === "rejected" && (
          <div className="mb-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="bg-white border-2 border-rose-100 rounded-[2rem] p-8 flex flex-col md:flex-row gap-6 items-start shadow-sm">
              <div className="p-4 bg-rose-50 rounded-2xl text-rose-500"><AlertCircle size={24} /></div>
              <div>
                <h3 className="text-xs font-black uppercase text-rose-900 mb-2 tracking-widest flex items-center gap-2">
                  <History size={14} /> Revision Required (Attempt #{indicator.rejectionCount})
                </h3>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          <div className="lg:col-span-8 space-y-12">
            <section className="grid grid-cols-2 md:grid-cols-4 gap-px bg-gray-200 border border-gray-200 rounded-[2rem] overflow-hidden shadow-sm">
              ...
              <StatCard label="Countdown" value={isOverdue ? "OVERDUE" : formatDuration(dueTime - now)} highlight={isOverdue ? "text-rose-500" : "text-emerald-600"} />
              <StatCard label="Measure Unit" value={indicator.unitOfMeasure} />
              <StatCard label="Audit Status" value={indicator.status} highlight={indicator.status === "rejected" ? "text-rose-600" : "text-[#C69214]"} />
            </section>
                ...
                {indicator.evidence.length ? (
                  <div className="divide-y divide-gray-50">
                    {indicator.evidence.map((file, idx) => (
                      <EvidenceRow key={`${file.publicId}-${idx}`} file={file} onPreview={() => setPreviewFile(file)} />
                    ))}
                  </div>
                ) : (
                  <div className="py-24 text-center">
                    ...
                    <p className="text-xs font-black text-gray-400 uppercase tracking-widest">Registry Empty</p>
                  </div>
                )}
              </div>
            </section>
          ...
          <aside className="lg:col-span-4">
            {canSubmit ? (
              <div className="sticky top-12">
                <div className={`bg-white rounded-[2.5rem] border shadow-xl p-8 relative overflow-hidden transition-colors ${isRevision ? "border-orange-100" : "border-gray-100"}`}>
                  <h2 className={`text-xs font-black uppercase tracking-widest mb-8 flex items-center gap-2 ${isRevision ? "text-orange-600" : ""}`}>
                    <Plus size={16} className={isRevision ? "text-orange-500" : "text-[#C69214]"} />
                    {isRevision ? "Submit Revision" : "Submit Assets"}
                  </h2>

                  <label className={`group flex flex-col items-center justify-center border-2 border-dashed rounded-3xl p-10 cursor-pointer transition-all duration-500 ${isRevision ? "border-orange-100 hover:border-orange-400 hover:bg-orange-50/30" : "border-gray-100 hover:border-[#C69214] hover:bg-emerald-50/10"}`}>
                    <div className={`w-12 h-12 rounded-2xl flex items-center justify-center mb-4 transition-all group-hover:scale-110 ${isRevision ? "bg-orange-50 text-orange-400 group-hover:text-orange-600" : "bg-gray-50 text-gray-400 group-hover:text-[#C69214]"}`}>
                      <Upload size={20} />
                    </div>
                    <span className="text-[10px] font-black uppercase tracking-widest text-gray-400 group-hover:text-[#1E3A2B]">Select Files / ZIP</span>
                  ...
                  <div className="mt-8 space-y-4 max-h-[350px] overflow-y-auto pr-2 custom-scrollbar">
                    {selectedFiles.map((file, i) => (
                      <div key={i} className="group bg-gray-50 rounded-2xl p-4 border border-transparent hover:border-gray-200 transition-all">
                  ...
                  >
                    ...
                    ) : isRevision ? "Push Final Revision" : "Verify & Submit"}
                  </button>
                </div>
              </div>
            ) : (
              <div className="p-12 text-center border-2 border-dashed border-gray-200 rounded-[3rem] bg-gray-50/50">
                ...
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest leading-loose">Registry Locked</p>
              </div>
            )}
          </aside>
        </div>
...
const Badge = ({ icon, label, color }: any) => (
  <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full border text-[9px] font-black uppercase tracking-widest ${color}`}>{icon} {label}</div>
);
...
const EvidenceRow = ({ file, onPreview }: { file: IEvidence; onPreview: () => void }) => (
    ...
    <div className="flex items-center gap-5">
      ...
      <div>
        ...
        <div className="flex items-center gap-2">
          <span className="text-[9px] font-black uppercase text-[#C69214] bg-orange-50 px-1.5 py-0.5 rounded">Exhibited</span>
          <p className="text-[10px] text-gray-400 font-medium">{file.description || "No context provided"}</p>
        </div>
      </div>
    </div>
    <button onClick={onPreview} className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest px-5 py-3 rounded-xl border border-gray-100 bg-white hover:bg-[#1E3A2B] hover:text-white hover:border-[#1E3A2B] transition-all duration-300 shadow-sm">
...
const LoadingRecords = () => (
  <div className="flex flex-col items-center justify-center h-screen bg-white">
    ...
    <p className="text-[10px] font-black uppercase tracking-[0.4em] text-gray-400 animate-pulse">Syncing Vault</p>
  </div>
);

const NotFound = ({ navigate }: any) => (
  <div className="flex flex-col items-center justify-center h-screen text-center p-6 bg-[#F8F9FA]">
    <div className="w-24 h-24 bg-white rounded-[2rem] flex items-center justify-center shadow-xl mb-8"><AlertCircle size={40} className="text-rose-200" /></div>
    <h2 className="text-2xl font-serif font-bold mb-4 text-[#1E3A2B]">Registry Link Broken</h2>
    <button onClick={() => navigate(-1)} className="group flex items-center gap-3 px-10 py-4 bg-[#1E3A2B] text-white text-[10px] font-black uppercase rounded-2xl tracking-[0.2em] shadow-2xl hover:-translate-y-1 transition-all">
      <ArrowLeft size={14} className="group-hover:-translate-x-1 transition-transform" /> Return to Index
    </button>
  </div>
);
