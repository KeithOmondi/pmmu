...
const SuperAdminSidebar = () => {
  ...
  const activeClass = 
    "bg-white/10 text-[#c2a336] shadow-sm ring-1 ring-white/20";

  const inactiveClass = 
    "text-gray-300 hover:bg-white/5 hover:text-white";

  return (
    <aside className={`sticky top-0 h-screen ${sidebarWidth} bg-[#1a3a32] text-white flex flex-col shadow-2xl transition-all duration-500 ease-in-out shrink-0 z-50`}>
      ...
      {/* Branding Section */}
      <div className={`p-6 flex flex-col items-center border-b border-white/10 transition-all duration-300 ${isCollapsed ? 'pt-8' : ''}`}>
        <div className={`bg-[#c2a336] rounded-full flex items-center justify-center shadow-lg ring-4 ring-[#c2a336]/20 transition-all duration-500 ${isCollapsed ? 'w-10 h-10 mb-0 rotate-0' : 'w-12 h-12 mb-4 rotate-[360deg]'}`}>
      ...
      {/* Navigation */}
      <nav className={`flex-1 overflow-y-auto overflow-x-hidden p-4 mt-4 space-y-1 custom-scrollbar`}>
        <NavItem 
        ...
        <div className={`transition-all duration-300 flex justify-center ${isCollapsed ? 'py-4' : 'pt-6 pb-2 px-4'}`}>
          <p className={`text-[10px] font-bold text-[#8c94a4] uppercase tracking-widest border-b border-white/5 pb-2 w-full transition-opacity ${isCollapsed ? 'opacity-0 h-0 overflow-hidden' : 'opacity-100'}`}>
            Configuration
          </p>
      ...
      <div className={`p-4 border-t border-white/10 space-y-2 transition-all duration-300`}>
        <button className={`flex items-center rounded-lg transition-colors group text-sm font-medium text-rose-300 hover:bg-rose-500/10 ${isCollapsed ? 'w-full justify-center p-3' : 'w-full px-4 py-3 gap-3'}`}>
          <LogOut size={18} className="shrink-0" />
          {!isCollapsed && <span className="animate-in slide-in-from-left-2">Sign Out</span>}
        </button>
        
        <div className={`bg-[#c2a336]/10 rounded-xl border border-[#c2a336]/20 transition-all duration-300 ${isCollapsed ? 'h-0 opacity-0 overflow-hidden p-0' : 'p-3 opacity-100'}`}>
...
const NavItem = ({ to, icon, label, isCollapsed, activeClass, inactiveClass, linkClass }: any) => (
  <NavLink
    to={to}
    className={({ isActive }) =>
